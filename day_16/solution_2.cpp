...
ll getEnergy(vector<string> &grid, vector<ll> start) {
    ll H = sz(grid);
    ll W = sz(grid[0]);

    deque<vector<ll>> points = {start};
...
int main() {
    ...
    ll res = 0;
    rep(x, H) {
        res = max(res, getEnergy(grid, {0, x, 0}));
        res = max(res, getEnergy(grid, {W - 1, x, 2}));
    }
    rep(y, W) {
        res = max(res, getEnergy(grid, {y, 0, 1}));
        res = max(res, getEnergy(grid, {y, H - 1, 3}));
    }
