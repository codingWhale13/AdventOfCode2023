...
int main() {
    ...
    ll galaxyExpansion = 1000000 - 1; // -1 because we REPLACE 1 empty col/row with 1000000
    ...
    rep(i, h) {
        rep(j, w) {
            if (grid[i][j] == '#')
                galaxies.push_back({
                    j + emptyColsPrefixSum[j] * galaxyExpansion,
                    i + emptyRowsPrefixSum[i] * galaxyExpansion,
                });
        }
    }
